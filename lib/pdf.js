...
function drawLabel(pdf, product, x, y, qrImage) {
  const width = A4_LAYOUT.labelWidth;
  const height = A4_LAYOUT.labelHeight;
  const padding = 4;
  const qrSize = 28;
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(9.5);
  const nameLines = pdf.splitTextToSize(cleanText(product.name, "Ürün"), textWidth);
  pdf.text(nameLines.slice(0, 2), textX, y + 16);
  ...
  if (product.shelfLocation) {
    pdf.text(`Raf: ${cleanText(product.shelfLocation)}`, textX, y + 35, { maxWidth: textWidth });
  }
...
export async function createPDF(products = []) {
  ...
  for (let index = 0; index < products.length; index += 1) {
    ...
    const qrImage = await generateQRCode(products[index].id);

    drawLabel(pdf, products[index], x, y, qrImage);
  }
