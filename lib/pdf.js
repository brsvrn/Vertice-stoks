...
function drawLabel(pdf, product, x, y, qrImage) {
  ...
  pdf.setFontSize(5.8);
  pdf.text("Envantra Stok Yönetim Sistemi", x + width / 2, y + height - 4, { align: "center" });
}
...
export async function createPDF(products = []) {
  ...
  const fileName = `Envantra-Etiket-${new Date().toISOString().slice(0, 10)}.pdf`;
  try {
    const base64Str = pdf.output("datauristring").split(',')[1];
    ...
    await Share.share({
      title: 'Envantra Etiket',
      url: result.uri,
      dialogTitle: 'Etiket PDF Kaydet veya Paylaş',
    });
  } catch (err) {
    console.error("PDF share failed, falling back to web save:", err);
